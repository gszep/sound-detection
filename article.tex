...
instruments based on the distribution of frequencies $\omega$ present, and thus
we choose $f$ to be a short-time fourier transform, where the
$\omega^\mathrm{th}$ component of the output represents the amplitude of
frequency $\omega$ at time $t$.
\begin{align}
	f(x,t)_\omega=
	\int_{-\infty}^{\infty}\!
...
For demonstration purposes a ~1/3s wide square window is chosen, which is on the
